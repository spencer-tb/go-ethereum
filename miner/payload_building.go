...
type BuildPayloadArgs struct {
	...
	BeaconRoot   *common.Hash      // The provided beacon root
}
...
func (w *worker) buildPayload(args *BuildPayloadArgs) (*Payload, error) {
	...
	empty, _, _, _, _, err := w.getSealingBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, args.BeaconRoot, true)
	if err != nil {
		return nil, err
	}
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				start := time.Now()
				block, fees, blobs, commits, proofs, err := w.getSealingBlock(args.Parent, args.Timestamp, args.FeeRecipient, args.Random, args.Withdrawals, args.BeaconRoot, false)
				if err == nil {
