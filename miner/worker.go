...
func (w *worker) makeEnv(parent *types.Header, header *types.Header, coinbase common.Address) (*environment, error) {
	...
	if header.BeaconRoot != nil {
		misc.ApplyBeaconRoot(header, state)
	}
...
type generateParams struct {
	...
	beaconRoot  *common.Hash      // The beacon root to include in block.
...
func (w *worker) getSealingBlock(parent common.Hash, timestamp uint64, coinbase common.Address, random common.Hash, withdrawals types.Withdrawals, beaconRoot *common.Hash, noTxs bool) (*types.Block, *big.Int, []kzg4844.Blob, []kzg4844.Commitment, []kzg4844.Proof, error) {
	req := &getWorkReq{
		params: &generateParams{
			timestamp:   timestamp,
			forceTime:   true,
