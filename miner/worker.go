...
func (w *worker) commitTransaction(env *environment, tx *txpool.Transaction) ([]*types.Log, error) {
	...
	if (len(env.blobs)+len(tx.Tx.BlobHashes()))*params.BlobTxBlobGasPerBlob > params.BlobTxMaxBlobGasPerBlock {
...
type generateParams struct {
	...
	beaconRoot  *common.Hash      // Parent beacon block root to set in precompile.
...
func (w *worker) prepareWork(genParams *generateParams) (*environment, error) {
	...
	if w.chainConfig.IsCancun(header.Number, header.Time) {
		// check if beacon root isn't nil because the mainLoop starts a
		// sealing task at startup
		if header.BeaconRoot != nil {
			context := core.NewEVMBlockContext(header, w.chain, nil)
			vmenv := vm.NewEVM(context, vm.TxContext{}, env.state, w.chainConfig, vm.Config{})
			env.state.AddAddressToAccessList(params.BeaconRootsStorageAddress)
			msg := &core.Message{
				...
				Data:      (*header.BeaconRoot)[:], // TODO use 4byte invocation?
			}
...
func (w *worker) getSealingBlock(args *BuildPayloadArgs, noTxs bool) (*types.Block, *big.Int, []kzg4844.Blob, []kzg4844.Commitment, []kzg4844.Proof, error) {
	req := &getWorkReq{
		params: &generateParams{
