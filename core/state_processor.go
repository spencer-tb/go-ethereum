...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	var (
		...
		allLogs     []*types.Log
		gp          = new(GasPool).AddGas(block.GasLimit()).AddBlobGas(params.MaxBlobGasPerBlock)
	)
	...
	for i, tx := range block.Transactions() {
		...
		receipt, err := applyTransaction(msg, p.config, gp, statedb, blockNumber, blockTime, blockHash, tx, usedGas, vmenv)
		if err != nil {
			return nil, nil, 0, fmt.Errorf("could not apply tx %d [%v]: %w", i, tx.Hash().Hex(), err)
		}
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, gp *GasPool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, usedGas *uint64, cfg vm.Config) (*types.Receipt, error) {
	...
	return applyTransaction(msg, config, gp, statedb, header.Number, header.Time, header.Hash(), tx, usedGas, vmenv)
}
